...
export const myMatchItemSchema = z
  .object({
    ...
    resultState: z.enum(["not-open", "reportable", "awaiting-opponent", "disputed", "finalized", "void"]),
  ...
  .superRefine((item, ctx) => {
    ...
    if (item.attention === "confirm-result" && item.resultState !== "awaiting-opponent") {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Confirm-result attention requires awaiting-opponent result state.", path: ["attention"] });
    }
    ...
    for (const [key, value] of Object.entries(page.summary)) {
      if (key !== "total" && value > page.summary.total) {
        ctx.addIssue({
          ...
          message: `${key} summary count cannot exceed total.`,
          path: ["summary", key],
        });
      }
    }
...
export class DjangoMyMatchesRepository implements MyMatchesRepository {
  async getMyMatches(query: MyMatchesQuery): Promise<MyMatchesPageData> {
    ...
    if (!configured) {
      throw new Error("VITE_API_BASE_URL is required when the Django My Matches adapter is enabled.");
    }
    ...
    if (!response.ok) {
      throw new MyMatchesHttpError(response.status);
    }
