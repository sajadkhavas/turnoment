...
>
> Every chat/agent/session that creates, rebuilds, productionizes, or materially edits a Turnoment frontend page MUST read this file and `PROJECT_CONTINUITY.md` before implementation. A page is built once as the final frontend implementation; architecture, SSR, routing, SEO/indexing, accessibility, responsive behavior, runtime contracts, error states, and quality gates are part of the page itself, not a later repair phase.
...
2. Verify current frontend `main` SHA.
3. Identify active/overlapping workstreams and shared-file ownership.
4. Create a dedicated branch from the verified SHA.
5. Record START_SHA and page scope.
...
Do not add schema merely to have schema.

### Private/account pages

Use explicit `noindex` policy and do not expose user-specific/private content as indexable landing pages.

No later generic "SEO phase" may be required to add the fundamentals above to pages created under this protocol. Site-wide SEO auditing can still refine content/strategy later without rebuilding page architecture.
...
- final SEO fundamentals implemented when public
